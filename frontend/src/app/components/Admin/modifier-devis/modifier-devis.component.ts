...
export class ModifierDevisComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.route.paramMap.subscribe(params => {
      ...
      this.selectedVehicule=this.devis.vehicule;
    });
  }
...
getDevis(devisId: string): void {
    ...
    this.vehiculeService.getVehiculesByClientId(clientId).subscribe(
      (data: any[]) => {
        ...
        if (this.vehicules.length > 0) {
          if (this.vehicules.length > 0) {
            const vehiculeExistant = this.vehicules.find(v => v._id === this.devis.vehicule?._id);
          
            if (vehiculeExistant) {
              this.selectedVehicule = vehiculeExistant;
            } else {
  ...
  updateDevis(): void {
    ...
    this.devisService.editDevis(this.devisId, updatedDevis).subscribe(
      response => {
        ...
        alert("Le devis a été modifié avec succès !");
      },
      error => {
  ...
  getTotalMontant(): number {
    return this.selectedTaches.reduce((total, tache) => total + tache.prix, 0);
  }
  
  getTotalDuree(): number {
    return this.selectedTaches.reduce((total, tache) => total + tache.tempsEstime + tache.marge, 0);
  }
